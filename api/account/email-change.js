// Node runtime — initiates email change: writes pending email to Sheets and
// emits a verification email via Gmail. SMTP latency lands this in Node 60s.
// Cookie in (auth required); Set-Cookie forwarded in case session rotates.
...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(`${base}/webhook/pkc-accounts/email/change`, {
      method: "POST",
      headers: {
