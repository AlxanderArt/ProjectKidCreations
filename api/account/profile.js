// Node runtime — GET reads the profile row from Sheets; PATCH writes back.
// Both routed to the same upstream path with method preserved so n8n's
// Webhook node can branch on req.method. Node 60s for Sheets jitter.
...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(`${base}/webhook/pkc-accounts/profile`, {
      method: req.method,
      headers: {
      ...
      ...(req.method === "PATCH" ? { body: bodyText } : {})
    });
