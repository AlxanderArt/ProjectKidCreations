// Edge runtime — request-reset stashes a token + Gmail send (~3s). Opaque
// by design (always 200), so Edge timeouts wouldn't change observable
// behavior much anyway. Moved off nodejs for the 12-function cap.
...
export default async function handler(request) {
  ...
  try {
    const upstream = await fetch(`${base}/webhook/pkc-accounts/password/request-reset`, {
      method: "POST",
      headers: {
