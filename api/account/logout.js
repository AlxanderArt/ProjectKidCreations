// Edge runtime — logout is a thin call: invalidate session, return an
// expiring Set-Cookie. No heavy upstream work, so Edge is fine here.
// Cookie in → upstream reads JTI to revoke; Set-Cookie out → clears the JWT.
...
export default async function handler(request) {
  ...
  try {
    const upstream = await fetch(`${base}/webhook/pkc-accounts/logout`, {
      method: "POST",
      headers: {
